...
$F(u) = \int_{-\infty}^{\infty} f(x) e^{-i 2 \pi fx} dx$, \greenbf{2D:} $F(u, v) = \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} f(x, y) e^{-2\pi i (ux, vy)}dx dy$\\
\greenbf{For images:} transformed image $\rightarrow$ $F = U * f$ $\leftarrow$ vectorized image, U: Fourier matrix\\
\greenbf{For discrete:} $F(u, v) = \sum_{x=0}^{N-1} \sum_{y=0}^{M-1} f(x, y) e^{-2\pi i (\frac{ux}{N}, \frac{vy}{M})}$\\
...
\greenbf{Convolution, Filtering:} The Fourier transfomr of the convolution of two functions is the product of their Fourier transform:\\
$F \cdot G = U(f ** g)$\\
\greenbf{Convolution, Sampling:} The Fourier transfomr of the product of two functions is the convolution of the Fourier transform.\\
$F ** G = U(f \cdot g)$\\
