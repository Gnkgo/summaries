...
\begin{compactitem}
    \item Parametric surfaces: \graytext{surfaces that are defined by a parameter space}
    \item Algebraic surfaces: \graytext{surface is zero set of pol.}
    \item Constructive solid geometry: \graytext{compl. shapes}
    \item Blobby surfaces: \graytext{blend surfaces toget.}
    \item Blending distance functions: \graytext{a distance function gives distance to closest point on object}
    \item Level set methods: \graytext{grid of values approx. func.}
    \item Fractals and L-systems: \graytext{self-similar}
\end{compactitem}
\greenbf{Geometry representations explicit}
\begin{compactitem}
    \item Point set: \graytext{collection of points that can be combined to surfaces}
    \item Point cloud: \graytext{list of points $(x, y, z)$, often augmented with normals}
    \item Polygonal mesh: \graytext{Store vertices and polygons, easier processing simulation, most common}
